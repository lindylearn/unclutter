<script lang="ts">
    ...
    export let updateMessage: string;

    const dispatch = createEventDispatcher();
</script>

<a class="feedback-container max-w-full px-2 py-2 pr-3 cursor-pointer relative rounded-lg shadow hover:shadow-md transition-all" href={`https://github.com/lindylearn/unclutter/releases/tag/${version}`} target="_blank" on:click={() => { dispatch("dismissed"); reportEventContentScript("clickUpdateMessage", {version}); }}>
    <svg class="close-message absolute top-1.5 right-2 w-2.5 opacity-70 text-gray-400 hover:text-gray-700 hover:scale-110 transition-all" viewBox="0 0 320 512" on:click={(e) => { e.preventDefault(); e.stopPropagation(); dispatch("dismissed"); reportEventContentScript("dismissedUpdateMessage", {version}); return false; }}>
        <path fill="currentColor" d="M310.6 361.4c12.5 12.5 12.5 32.75 0 45.25C304.4 412.9 296.2 416 288 416s-16.38-3.125-22.62-9.375L160 301.3L54.63 406.6C48.38 412.9 40.19 416 32 416S15.63 412.9 9.375 406.6c-12.5-12.5-12.5-32.75 0-45.25l105.4-105.4L9.375 150.6c-12.5-12.5-12.5-32.75 0-45.25s32.75-12.5 45.25 0L160 210.8l105.4-105.4c12.5-12.5 32.75-12.5 45.25 0s12.5 32.75 0 45.25l-105.4 105.4L310.6 361.4z"/>
    </svg>
    
    <div class="flex gap-2">
        <svg class="icon message-icon w-5 flex-shrink-0 mx-1" viewBox="0 0 512 512">
