...
<div
    class="article-card flex w-max flex-col gap-2 overflow-hidden rounded-xl bg-gradient-to-b from-yellow-200 to-yellow-300 p-1 text-sm text-stone-900 shadow drop-shadow"
    ...
    <div class="font-title flex items-stretch gap-1">
        ...
        <!-- {#await tags then tags}
            {#each tags.slice(0, 3) as tag, i}
                <div
                    class={clsx(
                        "tag cursor-pointer flex text-xs font-text px-2 py-1 gap-1 rounded-lg bg-white"
                        // i === 0 && "rounded-l-lg",
        ...
        <div
            class="lindy-button cursor-pointer rounded-r-lg px-2 py-1 font-bold transition-all"
...
>
    <div class="font-text p-2">
        {articleSummary ||
            "Sammy screens a movie during prom that he shot of his classmates. The project’s apparent “hero” is Logan, a Teutonic athlete whom Sammy depicts as a golden god, even though Logan has tormented him all year."}
    </div>
</div> -->
<style lang="postcss">
    @tailwind base;
