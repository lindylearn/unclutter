...
<a
    ...
    on:click={() => {
        dispatch("dismissed");
        reportEventContentScript("clickUpdateMessage", { version });
    }}
>
    <svg
        ...
        on:click={(e) => {
            ...
            e.stopPropagation();
            dispatch("dismissed");
            reportEventContentScript("dismissedUpdateMessage", { version });
            return false;
        }}
    >
        <path
    ...
    <div class="flex gap-2">
        <svg class="icon message-icon mx-1 w-5 flex-shrink-0" viewBox="0 0 512 512">
            <path
                fill="currentColor"
