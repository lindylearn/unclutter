...
>
    <div
        class="flex h-[calc(1rem+0.5rem+1.25rem+0.75rem*2)] items-center justify-between gap-5 rounded-lg p-3 px-4"
    >
        <div class="main-content whitespace-nowrap text-sm">
            {#if libraryState.feed}
                ...
                >
                    Follow <span class="hide-tiny">{libraryState.feed.domain}</span>
                </div>
            {/if}

            {#if libraryState.feed?.postFrequency}
                <div
                    class={clsx(
                ...
                >
                    about {libraryState.feed?.postFrequency}
                </div>
            {/if}
        </div>

        <svg class="w-6 shrink-0" viewBox="0 0 448 512"
            ><path
                fill="currentColor"
